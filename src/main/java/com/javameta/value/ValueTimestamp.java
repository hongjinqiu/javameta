...
public class ValueTimestamp extends Value {
    ...
    public static ValueTimestamp get(java.util.Date date) {
    	return get(new Timestamp(date.getTime()));
    }
    ...
    public static ValueTimestamp parse(String s) {
        try {
        	...
        	throw new RuntimeException("INVALID_DATETIME_CONSTANT TIMESTAMP:" + s, e);
        }
    }
    ...
    public Value convertScale(boolean onlyToSmallerScale, int targetScale) {
        ...
        if (targetScale < 0) {
        	throw new RuntimeException("InvalidValue scale:" + targetScale);
        }
        long n = timeNanos;
