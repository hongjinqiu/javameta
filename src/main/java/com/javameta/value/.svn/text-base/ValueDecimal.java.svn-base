...
import com.javameta.util.MathUtils;
...
public class ValueDecimal extends Value {
    ...
    private ValueDecimal(BigDecimal value) {
        if (value == null) {
        	...
        	throw new RuntimeException("INVALID_CLASS expect " + BigDecimal.class.getName() + " but get " + value.getClass().getName());
        }
        this.value = value;
    }
    ...
    public Value divide(Value v) {
        ...
        if (dec.value.signum() == 0) {
        	throw new RuntimeException("DIVISION_BY_ZERO:" + getSQL());
        }
    ...
    public Value convertPrecision(long precision, boolean force) {
        ...
        throw new RuntimeException("NUMERIC_VALUE_OUT_OF_RANGE:" + Long.toString(precision));
    }
    ...
    public static BigDecimal setScale(BigDecimal bd, int scale) {
        if (scale > BIG_DECIMAL_SCALE_MAX || scale < -BIG_DECIMAL_SCALE_MAX) {
        	throw new RuntimeException("InvalidValue scale:" + scale);
        }
