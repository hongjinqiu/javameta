...
<td width="${columnItemForJsp.columnWidth}" colspan="${columnItemForJsp.columnSpan}">
	<div id="${columnModelForJsp.name}_${columnModelForJsp.dataSetId}_${columnItemForJsp.name}_render_{index}">
		...
		<c:if test="${editorName == 'textfield'}">
			<input id="${columnModelForJsp.name}_${columnModelForJsp.dataSetId}_${columnItemForJsp.name}_{index}" name="${columnItemForJsp.name}" type="text" class="${columnItemForJsp.column.editorFieldCls}" style="${columnItemForJsp.column.editorStyle}" />
		</c:if>
		<c:if test="${editorName == 'textareafield'}">
			<textarea id="${columnModelForJsp.name}_${columnModelForJsp.dataSetId}_${columnItemForJsp.name}_{index}" name="${columnItemForJsp.name}" class="${columnItemForJsp.column.editorFieldCls}" style="${columnItemForJsp.column.editorStyle}"></textarea>
		</c:if>
		...
		<c:if test="${editorName == 'datefield'}">
			...
			<c:if test="${columnItemForJsp.column.dbPattern == 'HHmmss'}">
				<input id="${columnModelForJsp.name}_${columnModelForJsp.dataSetId}_${columnItemForJsp.name}_{index}" name="${columnItemForJsp.name}" class="${columnItemForJsp.column.editorFieldCls}" style="${columnItemForJsp.column.editorStyle}" />
			</c:if>
		</c:if>
		...
		<c:if test="${editorName == 'displayfield'}">
			<span id="${columnModelForJsp.name}_${columnModelForJsp.dataSetId}_${columnItemForJsp.name}_{index}" name="${columnItemForJsp.name}" class="${columnItemForJsp.column.editorFieldCls}" style="${columnItemForJsp.column.editorStyle}">
			</span>
		</c:if>
		...
		<c:if test="${editorName == 'triggerfield'}">
			<input id="${columnModelForJsp.name}_${columnModelForJsp.dataSetId}_${columnItemForJsp.name}_{index}" name="${columnItemForJsp.name}" type="text" class="${columnItemForJsp.column.editorFieldCls}" style="${columnItemForJsp.column.editorStyle}" />
			<input type="button" id="${columnModelForJsp.name}_${columnModelForJsp.dataSetId}_${columnItemForJsp.name}_{index}_select" name="selectBtn" value="选择" />
			<input type="button" id="${columnModelForJsp.name}_${columnModelForJsp.dataSetId}_${columnItemForJsp.name}_{index}_view" name="viewBtn" value="查看" />
			<input type="button" id="${columnModelForJsp.name}_${columnModelForJsp.dataSetId}_${columnItemForJsp.name}_{index}_delete" name="deleteBtn" value="删除" />
		</c:if>
	</div>
	<script type="text/javascript">
	g_gridCommondDict["${columnModelForJsp.dataSetId}"].push(function(index) {
		var formFieldFactory = new FormFieldFactory();
		var fieldId = "${columnModelForJsp.name}_${columnModelForJsp.dataSetId}_${columnItemForJsp.name}_{index}";
		fieldId = fieldId.replace(/{index}/g, index);
		var field = formFieldFactory.getFormField(fieldId, '${columnItemForJsp.name}', '${columnModelForJsp.dataSetId}');
		return field;
	});
	</script>
</td>
