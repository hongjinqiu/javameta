			...
			if (columnLi[i].relationDS && columnLi[i].relationDS.relationItem) {
				for (var j = 0; j < columnLi[i].relationDS.relationItem.length; j++) {
					...
					if (relationItem.copyConfig) {
						for (var k = 0; k < relationItem.copyConfig.length; k++) {
							var copyColumnName = relationItem.copyConfig[k].copyDestField;
							var isIn = false;
				...
				if (j != i) {
					if (columnLi[j].relationDS && columnLi[j].relationDS.relationItem) {
						for (var k = 0; k < columnLi[j].relationDS.relationItem.length; k++) {
							...
							if (relationItem.copyConfig) {
								for (var l = 0; l < relationItem.copyConfig.length; l++) {
									var copyColumnName = relationItem.copyConfig[l].copyDestField;
									var isIn = false;
